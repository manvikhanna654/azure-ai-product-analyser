...
def render_upload() -> None:
    ...
    st.markdown(
        '<div class="pv-eyebrow" style="margin-top:.4rem">Upload product image</div>',
    ...
    upload = st.file_uploader(
        ...
        help="JPG, JPEG or PNG up to 10 MB.",
    )

    st.markdown(
        '<div style="display:flex;gap:.5rem;margin-top:.7rem;flex-wrap:wrap">'
        '<span class="pv-pill">Formats: JPG, JPEG, PNG</span>'
    ...
    if upload is None:
        ...
        with left:
            theme.card(
                "Before you upload",
                "Fill the frame with a single product, keep the background plain "
                "and avoid heavy shadows. Sharper images give cleaner attributes.",
            )
        with right:
            theme.card(
                "What you get back",
                "A product overview, a breakdown of visual attributes, a written "
                "description and a question panel tied to the same image.",
            )
        st.button("Analyze product", type="primary", disabled=True)
    ...
    if size_mb > MAX_MB:
        st.error(
            f"That file is {size_mb:.1f} MB. Upload an image under {MAX_MB} MB."
        )
        return
    ...
    with right:
        theme.section("Product preview", eyebrow="Ready to analyse")
        theme.rows(
            [
                ("File name", upload.name),
                ("Dimensions", f"{image.width} × {image.height} px"),
                ("File size", _readable_size(upload.size)),
                ("Format", (image.format or upload.type.split("/")[-1]).upper()),
            ]
        )
        theme.spacer(0.8)
